...
def parse_xml_example(sp):
    ...
    text = 'CYP3A4 mRNA expression was significantly increased by rifampicin exposure in human hepatocytes.'
    print 'IN:', text
...
def get_dependencies_example(sp):
    ...
    for td in sentence.gs.allTypedDependencies():
        ...
        #print tmpl % (gov.value(), dep.value(), rel)
    print '-' * 80


def get_common_path_example(sp,text,i,j):
    tmpl = 'Least common node for "%s" and "%s" '
    print 'Common path:'
    print 'Text:', text
    sentence = sp.parse(text)
    gov = []
	...
	Gov = td.gov()
        gov.append(Gov)
        Gov_idx = Gov.index()
        gov_idx.append(Gov_idx)
        Dep = td.dep()
        dep.append(Dep)
        Dep_idx = Dep.index()
        dep_idx.append(Dep_idx)
        Rel = td.reln()
        rel.append(Rel)	
    lcn, shortest_path = sentence.get_least_common_node(i, j)
    l = []
    for a in range(len(sorted(shortest_path))-1):
        b = a + 1
        x = sorted(shortest_path)[a]
        y = sorted(shortest_path)[b]
        if (x in dep_idx) and (y in gov_idx):
             if dep_idx.index(x) == gov_idx.index(y):
                   l.append(rel[dep_idx.index(x)])
                   l.append(sentence.word[y])
    print l
...
if __name__ == '__main__':
    ...
    text = f.readline()
    #text = 'CYP3A4 mRNA expression was significantly increased by rifampicin exposure in human hepatocytes.'
    #text2 = 'Clonidine noncompetitively inhibited acetylcholinesterase activity in vitro and after in vivo administration at protective doses.'

    #parse_xml_example(sp)
    #parse_probabilities_example(sp)
    #subtrees_example(sp)
    get_dependencies_example(sp)
    get_common_path_example(sp,text,i,j)
